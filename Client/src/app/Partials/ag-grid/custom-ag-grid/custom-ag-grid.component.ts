...
export class CustomAgGridComponent implements OnInit {
  ...
  getSelectedRows() {
    const selectedData = this.agGrid.api.getSelectedNodes().map((node:any) => {
      if (node.groupData) {
  ...
  onSelectionChanged($event:any) {
    ...
    {
       this.downloadData.emit(selectedRows[0]);
      this.localStorageService.removeItem("agGridDownload");
  ...
  deleteItem() {
    ...
    var deger = this.selectedData.map((node :any) => node.make).join(',');
    alert(deger)
  ...
  secme() {
    ...
    this.selectedData = this.agGrid.api.getSelectedNodes().map((node :any)  => {
      if (node.groupData) {
        return { make: node.key, model: 'Group' };
      }
      return node.data;
  ...
  //     { field: '' ,   headerName:this.translate.instant("TEXT."),  minWidth: 150,hide:true },
  //     { field: '' ,   headerName:this.translate.instant("TEXT."),  minWidth: 150 },
